...
window.ExpenseTracker = {
  ...
  updateBalanceDisplay(retries = 3) {
    const attemptUpdate = (attempt) => {
      try {
        const dashboardBalance = document.getElementById("dashboardBalance");
        if (dashboardBalance) {
          dashboardBalance.textContent = Utils.formatCurrency(this.stats.monthlyBalance);
          console.log(`Dashboard balance updated successfully on attempt ${attempt + 1}:`, this.stats.monthlyBalance);
          return true;
        } else {
          console.warn(`Dashboard balance element not found on attempt ${attempt + 1}`);
          return false;
        }
      } catch (error) {
        console.error(`Error updating balance on attempt ${attempt + 1}:`, error);
        return false;
      }
    };

    // Coba update langsung
    if (attemptUpdate(0)) return;

    // Jika gagal, coba dengan delay
    for (let i = 1; i < retries; i++) {
      setTimeout(() => {
        if (!attemptUpdate(i)) {
          console.error(`Failed to update dashboard balance after ${i + 1} attempts`);
        }
      }, i * 50);
    }
  },
  ...
  addTransaction() {
    ...
    setTimeout(() => {
      ...
      // Enhanced dashboard balance update dengan error handling
      try {
        const dashboardBalance = document.getElementById("dashboardBalance");
        if (dashboardBalance) {
          dashboardBalance.textContent = Utils.formatCurrency(this.stats.monthlyBalance);
        } else {
          console.warn("Dashboard balance element not found during transaction add");
        }

        // Update dashboard stats elements dengan error checking
        ...
        if (monthIncomeElement) {
          monthIncomeElement.textContent = Utils.formatCurrency(this.stats.monthlyIncome);
        }

        if (monthExpenseElement) {
          monthExpenseElement.textContent = Utils.formatCurrency(this.stats.monthlyExpense);
        }

        // Also call updateDashboardStats for any other elements
  ...
  updateDashboardStats() {
    try {
      ...
      // Update dashboard balance element (prioritas utama)
      const dashboardBalance = document.getElementById("dashboardBalance");
      if (dashboardBalance) {
        dashboardBalance.textContent = Utils.formatCurrency(stats.monthlyBalance);
        console.log("Dashboard balance updated:", stats.monthlyBalance);
      } else {
        console.warn("Dashboard balance element not found in updateDashboardStats");
      }

      // Update elemen yang ada di dashboard.html
      const monthIncomeElement = document.getElementById("monthIncome");
      const monthExpenseElement = document.getElementById("monthExpense");

      if (monthIncomeElement) {
        monthIncomeElement.textContent = Utils.formatCurrency(stats.monthlyIncome);
        console.log("Month income updated:", stats.monthlyIncome);
      }

      if (monthExpenseElement) {
        monthExpenseElement.textContent = Utils.formatCurrency(stats.monthlyExpense);
        console.log("Month expense updated:", stats.monthlyExpense);
      }

      // Fallback: jika ada statsContainer (untuk compatibility)
  ...
  updateBalance() {
    // Update main dashboard balance only (header balance removed)
    const dashboardBalance = document.getElementById("dashboardBalance");

    const formattedBalance = Utils.formatCurrency(this.stats.monthlyBalance);

    if (dashboardBalance) {
      dashboardBalance.textContent = formattedBalance;
      console.log("Dashboard balance updated:", this.stats.monthlyBalance);
    } else {
      console.warn("Dashboard balance element not found");
    }
  },
