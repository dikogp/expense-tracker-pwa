...
window.ExpenseTracker = {
  ...
  setBudget() {
    ...
    if (amount >= 0) {
      console.log(`Setting budget for ${currentMonth}: ${Utils.formatCurrency(amount)}`);
      ...
      <div class="budget-overview-content">
        <div class="budget-header-modern">
          <div class="budget-amount-display">
            ...
            <span class="budget-amount-large">${Utils.formatCurrency(stats.monthlyBudget)}</span>
          </div>
        ...
        <div class="budget-progress-modern">
          <div class="progress-info">
            ...
            <span class="progress-amount">${Utils.formatCurrency(stats.monthlyExpense)}</span>
          </div>
          <div class="progress-bar-container">
            <div class="progress-bar-modern">
              <div class="progress-fill-modern ${stats.budgetPercentage > 100 ? 'over-budget' : ''}" 
                   style="width: ${Math.min(stats.budgetPercentage, 100)}%"></div>
            </div>
            <span class="progress-percentage">${stats.budgetPercentage.toFixed(1)}%</span>
          </div>
          <div class="budget-remaining-info ${stats.budgetRemaining < 0 ? 'negative' : 'positive'}">
            <span class="remaining-label">${stats.budgetRemaining < 0 ? 'Over budget by' : 'Remaining'}</span>
            <span class="remaining-amount">${Utils.formatCurrency(Math.abs(stats.budgetRemaining))}</span>
          </div>
        </div>
      </div>
    `;
