...
const ExpenseTracker = {
  ...
  updateAnalytics() {
    ...
    const transactions = this.transactions.filter(t => t.date.startsWith(thisMonth));
    ...
    analyticsContent.innerHTML = `
      <div class="analytics-summary">
        ...
        <div class="stat-card">
          <h3>Selisih</h3>
          <p class="${stats.income - stats.expense >= 0 ? "income" : "expense"}">
  ...
  initializeAnalyticsCharts(expenseByCategory, dailyExpense) {
    ...
    if (categoryCtx) {
      new Chart(categoryCtx, {
        type: "pie",
        data: {
          labels: Object.keys(expenseByCategory).map(cat => 
            this.categories.expense?.[cat]?.name || cat
          ),
          datasets: [{
            data: Object.values(expenseByCategory),
            backgroundColor: Object.keys(expenseByCategory).map(cat =>
              this.categories.expense?.[cat]?.color || "#666"
            )
          }]
        },
        options: {
