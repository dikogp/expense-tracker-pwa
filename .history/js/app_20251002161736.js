...
window.ExpenseTracker = {
  ...
  addTransaction() {
    ...
    // Pastikan data tersimpan dan sinkron sebelum update UI
    this.updateGlobalStats();

    // Tampilkan dashboard
    this.showSection("dashboard");

    // Update UI dengan sedikit delay untuk memastikan DOM sudah ready
    setTimeout(() => {
      this.updateUI();
    }, 50);
  ...
  handleQuickAdd() {
    ...
    if (!this._lastAddQuickNotif || this._lastAddQuickNotif !== notifMessage) {
      ...
      setTimeout(() => {
        if (this._lastAddQuickNotif === notifMessage)
          this._lastAddQuickNotif = null;
      }, 1200);
    }
  },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.NotificationSystem = window.NotificationSystem || {
    addNotification: function (type, message, amount) {
      ...
      if (this._lastNotif === notifKey && Date.now() - this._lastNotifTime < 1000) {
