...
const ExpenseTracker = {
  ...
  exportUserData() {
    ...
    a.href = url;
    a.download = `pengeluaranqu-data-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  ...
  renderProfileContent() {
    ...
    // Get the first letter of the name for the avatar
    const initial = (this.currentUser.name || "Pengguna").charAt(0).toUpperCase();
    ...
    profileContent.innerHTML = `
      <div class="profile-container">
        <div class="profile-header">
          ...
          <div class="profile-info">
            <h2 class="profile-name">${this.currentUser.name || "Pengguna"}</h2>
            <p class="profile-email">${this.currentUser.email || "pengguna@tamu"}</p>
            ${this.currentUser.isGuest ? 
              `<span class="guest-badge">
                <i class="fas fa-user-clock"></i>
                Mode Tamu
        ...
        <div class="profile-stats">
          ...
          <div class="stat-card">
            <div class="stat-title">Total Pemasukan</div>
            <div class="stat-value income">${Utils.formatCurrency(stats.totalIncome)}</div>
          </div>

          <div class="stat-card">
            <div class="stat-title">Total Pengeluaran</div>
            <div class="stat-value expense">${Utils.formatCurrency(stats.totalExpense)}</div>
          </div>
        </div>
        ...
        <div class="activity-section">
          <div class="activity-header">
            <h3>Aktivitas Terakhir</h3>
            ${recent.length > 0 ? '<button class="btn-text" onclick="ExpenseTracker.showSection(\'history\')">Lihat Semua</button>' : ''}
          </div>
          <div class="activity-list">
        ...
        <div class="profile-actions">
          ${!this.currentUser.isGuest ? `
            ...
            <button class="profile-button" onclick="window.openModal('settingsModal')">
              <i class="fas fa-cog"></i>
              Pengaturan
            </button>
          ` : ''}
