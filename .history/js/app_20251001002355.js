...
const ExpenseTracker = {
    ...
    if (statsContainer) {
      statsContainer.innerHTML = `
        ...
        <div class="stat-card balance">
          ...
          }">${Utils.formatCurrency(balance)}</div>
        </div>
      `;
    }
  },
  ...
  updateBudget() {
    ...
    const monthBudget = this.budget[thisMonth] || 0;
    const monthExpense = this.transactions
      .filter(t => t.type === 'expense' && t.date.startsWith(thisMonth))
      .reduce((sum, t) => sum + t.amount, 0);
    
    const remaining = monthBudget - monthExpense;
    const percentage = monthBudget > 0 ? Math.min(100, Math.round((monthExpense / monthBudget) * 100)) : 0;
    ...
    budgetSection.innerHTML = `
      ...
      <div class="budget-info">
        ...
        <div class="budget-amount">${Utils.formatCurrency(monthBudget)}</div>
      </div>
      <div class="budget-progress">
        ...
        <div class="progress-labels">
          <span class="progress-percent">${percentage}% digunakan</span>
          <span class="progress-value">${Utils.formatCurrency(monthExpense)} / ${Utils.formatCurrency(monthBudget)}</span>
        </div>
      </div>
      <div class="budget-remaining ${remaining < 0 ? 'negative' : ''}">
        <span>Sisa: ${Utils.formatCurrency(remaining)}</span>
      </div>
    `;
  },
  ...
  updateBudgetStatus() {
    ...
    } else if (percentage <= 25 && monthBudget > 0) {
      statusClass = 'good';
    ...
    budgetStatus.innerHTML = `
      ...
      <div class="status-details">
        ...
        <span>${Utils.formatCurrency(monthExpense)} / ${Utils.formatCurrency(monthBudget)}</span>
      </div>
    `;
  },

  calculateUserStats() {
    // Menggunakan statistik bulan berjalan untuk konsistensi dengan dashboard
    const thisMonth = new Date().toISOString().substring(0, 7);
    const monthTransactions = this.transactions.filter((t) =>
      t.date.startsWith(thisMonth)
    );
    ...
    const stats = {
      // Total statistik untuk keseluruhan
      ...
      // Budget bulan ini
      monthlyBudget: this.budget[thisMonth] || 0,
