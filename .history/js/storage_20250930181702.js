...
const Storage = {
  ...
  exportAsJSON: function (data) {
    ...
    a.href = url;
    a.download = `pengeluaranqu-backup-${
  ...
  exportAsCSV: function (expenses) {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `pengeluaranqu-expenses-${
      new Date().toISOString().split("T")[0]
    }.csv`;
    document.body.appendChild(a);
    a.click();
  ...
  init: function () {
    this.performSecurityChecks();
    this.cleanupOldData();
    console.log("Enhanced Storage module initialized with security features");
  },
  ...
  sanitizeExpenseData: function (expenses) {
    ...
    return expenses
      .map((expense) => ({
        ...
        type: ["income", "expense"].includes(expense.type)
          ? expense.type
          : "expense",
        ...
        tags: Array.isArray(expense.tags)
          ? expense.tags.map((tag) => this.sanitizeString(tag))
          : [],
      }))
    ...
    if (user && !PremiumFeatures?.isPremiumUser(user)) {
      ...
      this.loadExpenses().then((expenses) => {
        const filteredExpenses = expenses.filter(
          (expense) => new Date(expense.date) > twoYearsAgo
        );
        ...
        try {
          ...
          if (Array.isArray(expenses)) {
            ...
            const migratedExpenses = expenses.map((expense) => ({
              ...
              timestamp:
                expense.timestamp ||
                new Date(`${expense.date}T${expense.time}`).getTime(),
            }));
...
// Initialize enhanced storage on load
