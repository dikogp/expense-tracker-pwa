...
window.ExpenseTracker = {
  ...
  bindEvents() {
    ...
    const amountInputs = ["amount", "quickAddAmount", "editAmount", "monthlyBudget", "categoryBudgetAmount"]; // ditambah edit & budget inputs
  ...
  addTransaction() {
    ...
    // Tampilkan dashboard lebih dulu agar transisi terasa cepat
    this.showSection("dashboard");
    // Perbarui UI penuh
    this.updateUI();

    const typeText = transaction.type === "income" ? "pemasukan" : "pengeluaran";
    const notifMessage = `${typeText} ${transaction.description ? `(${transaction.description})` : ""} berhasil ditambahkan`;

    if (!this._lastAddNotif || this._lastAddNotif !== notifMessage) {
      this._lastAddNotif = notifMessage;
      NotificationSystem.addNotification("transaction", notifMessage, transaction.amount);
      setTimeout(() => {
        if (this._lastAddNotif === notifMessage) this._lastAddNotif = null;
      }, 1200);
    }
  },
  ...
  handleQuickAdd() {
    ...
    const typeText = transaction.type === "income" ? "pemasukan" : "pengeluaran";
    const notifMessage = `${typeText} cepat ${transaction.description ? `(${transaction.description})` : ""} berhasil ditambahkan`;
    if (!this._lastAddQuickNotif || this._lastAddQuickNotif !== notifMessage) {
      this._lastAddQuickNotif = notifMessage;
      NotificationSystem.addNotification("transaction", notifMessage, transaction.amount);
      setTimeout(() => {
        if (this._lastAddQuickNotif === notifMessage) this._lastAddQuickNotif = null;
      }, 1200);
    }
  },
  ...
  setBudget() {
    ...
    const formData = new FormData(form);
    const amountInput = document.getElementById("monthlyBudget") || document.getElementById("budget-amount");
    const raw = amountInput ? (amountInput.dataset.rawValue || amountInput.value || "") : (formData.get("budget") || "");
  ...
  showToast(message, type = "info") {
    if (this._lastToast && this._lastToast.message === message && Date.now() - this._lastToast.time < 900) {
