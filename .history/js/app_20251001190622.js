...
const ExpenseTracker = {
  ...
  updateHistory() {
    ...
    const totalExpenseFiltered = document.getElementById('totalExpenseFiltered');
    const netAmountFiltered = document.getElementById('netAmountFiltered');
    
    if (!transactionList) return;
    
    // Get all transactions sorted by date (newest first)
    const transactions = [...this.transactions].sort((a, b) => b.timestamp - a.timestamp);
    ...
    // Update summary
    if (totalIncomeFiltered) totalIncomeFiltered.textContent = Utils.formatCurrency(totalIncome);
    if (totalExpenseFiltered) totalExpenseFiltered.textContent = Utils.formatCurrency(totalExpense);
    ...
    transactionList.innerHTML = filteredTransactions.map(t => `
      <div class="transaction-item ${t.type}" data-id="${t.id}">
        ...
        <div class="transaction-actions">
          <button class="btn-icon edit-transaction" onclick="ExpenseTracker.editTransaction('${t.id}')">✏️</button>
          <button class="btn-icon delete-transaction" onclick="ExpenseTracker.deleteTransaction('${t.id}')">🗑️</button>
        </div>
      </div>
    `).join('');
  },
