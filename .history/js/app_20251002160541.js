...
window.ExpenseTracker = {
  ...
  bindEvents() {
    ...
    const amountInputs = ["amount", "quickAddAmount"];
  ...
  addTransaction() {
    ...
    // Clean the amount value before parsing
    const amountValue = (formData.get("amount") || "").replace(/[^0-9]/g, "");
  ...
  setBudget() {
    ...
    const formData = new FormData(form);
    const amountInput = document.getElementById("monthlyBudget");
    const amountValue = amountInput.dataset.rawValue || amountInput.value;
    const amount = parseFloat(amountValue);
    const monthInput = document.getElementById("budget-month");
    const currentMonth =
