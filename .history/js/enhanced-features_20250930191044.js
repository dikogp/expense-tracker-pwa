...
const EnhancedFeatures = {
  ...
  Analytics: {
    ...
    calculateCategoryBreakdown: function(expenses) {
      ...
      return Object.keys(categories).map(categoryKey => ({
        ...
        average: categories[categoryKey].total / Math.max(1, Object.keys(categories[categoryKey].monthly).length)
    ...
    calculatePredictions: function(expenses) {
      ...
      const averageMonthly = expenses.length > 0 ? 
        expenses.reduce((sum, e) => sum + e.amount, 0) / 6 : 0;
    ...
    renderTrendChart: function() {
      ...
      // Update insights
      document.getElementById('monthlyTrend').textContent = this.analyticsData.insights.monthlyTrend;
      document.getElementById('trendRecommendation').textContent = this.analyticsData.insights.recommendation;
    },
      ...
      container.innerHTML = categories.map(cat => `
        <div class="category-analysis-item">
          <div class="category-header">
            <h4>${cat.category}</h4>
            <span class="category-total">${Utils.formatCurrency(cat.total)}</span>
          </div>
  ...
  CategoryBudgets: {
    ...
    renderBudgetsList: async function() {
      ...
      if (Object.keys(this.budgets).length === 0) {
        container.innerHTML = `
          <div class="empty-state">
            ...
            <small>${PremiumFeatures.isPremiumUser() ? 
      ...
      const budgetItems = Object.keys(this.budgets).map(categoryKey => {
        const budget = this.budgets[categoryKey];
        const spent = this.calculateSpent(expenses, categoryKey, budget.period);
    ...
    renderBudgetItem: function(categoryKey, budget, spent, percentage) {
      const user = Auth.getCurrentUser();
      const categoryName = user?.settings?.categories?.expense?.[categoryKey]?.name || categoryKey;
      const categoryIcon = user?.settings?.categories?.expense?.[categoryKey]?.icon || '📂';
      ...
      return `
        <div class="category-budget-item">
          <div class="budget-header">
            ...
            <div class="budget-amount">${Utils.formatCurrency(budget.amount)}/${this.getPeriodText(budget.period)}</div>
          </div>
          ...
          <div class="budget-status">
            ...
            <span class="${isOverBudget ? 'text-danger' : ''}">${percentage.toFixed(1)}%</span>
          </div>
...
function exportAnalytics(format) {
  ...
  showToast(`Mengekspor laporan analisis ke format ${format.toUpperCase()}...`, 'info');
