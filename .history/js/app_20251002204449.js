...
window.ExpenseTracker = {
  ...
  addTransaction() {
    ...
    // Switch to dashboard first, then update content
    console.log("Switching to dashboard...");
    this.showSection("dashboard");

    // Give the DOM time to render the dashboard, then update its content
    setTimeout(() => {
      console.log("Updating dashboard content after section switch");
      this.updateDashboard();
    }, 150);
  },
  ...
  updateDashboardStats() {
    ...
    const attemptUpdate = (attempt = 0) => {
      try {
        console.log(`updateDashboardStats: Attempt ${attempt + 1} of ${maxRetries}`);
        ...
        const dashboardSection = document.getElementById("dashboard");
        if (!dashboardSection) {
          console.log("updateDashboardStats: Dashboard section not found");
        ...
        const dashboardBalance = document.getElementById("dashboardBalance");
        if (!dashboardBalance) {
          console.log(`updateDashboardStats: dashboardBalance element not found on attempt ${attempt + 1}`);
          if (attempt < maxRetries - 1) {
            setTimeout(() => attemptUpdate(attempt + 1), retryDelay);
            return;
          }
          console.error("updateDashboardStats: Failed to find dashboardBalance after all attempts");
          return;
        }
        ...
        for (const [elementId, data] of Object.entries(dashboardElements)) {
          ...
          if (element) {
            ...
            updateCount++;
            console.log(
              `updateDashboardStats: Updated ${elementId} to ${formattedValue}`
            );
          } else {
            console.warn(
              `updateDashboardStats: Element ${elementId} not found in DOM`
            );
          }
        }
        
        console.log(`updateDashboardStats: Successfully updated ${updateCount} elements`);
        return true; // Success
        
      } catch (error) {
        console.error(`updateDashboardStats: Error on attempt ${attempt + 1}:`, error);
        if (attempt < maxRetries - 1) {
          setTimeout(() => attemptUpdate(attempt + 1), retryDelay);
          return;
        }
