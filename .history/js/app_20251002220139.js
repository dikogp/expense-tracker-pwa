...
window.ExpenseTracker = {
    ...
    setTimeout(() => {
      switch (sectionId) {
        case "dashboard":
          ...
          break;
        case "history":
          // Ensure data is loaded and stats are calculated before updating history
          this.updateGlobalStats();
          setTimeout(() => {
            this.updateHistory();
          }, 100);
  ...
  showBudgetForm() {
    ...
    const currentBudget = this.budget[currentMonth] || 0;

    if (budgetInput) {
      if (currentBudget > 0) {
        budgetInput.value = currentBudget.toLocaleString("id-ID");
        budgetInput.dataset.rawValue = currentBudget.toString();
      } else {
        budgetInput.value = "";
    ...
    newMonthInput.addEventListener("change", () => {
      const selectedMonth = newMonthInput.value;
      const monthBudget = this.budget[selectedMonth] || 0;
      const budgetAmountInput = document.getElementById("budget-amount");

      if (budgetAmountInput) {
        if (monthBudget > 0) {
          budgetAmountInput.value = monthBudget.toLocaleString("id-ID");
          budgetAmountInput.dataset.rawValue = monthBudget.toString();
        } else {
          budgetAmountInput.value = "";
          budgetAmountInput.dataset.rawValue = "";
        }
      }
    });
  ...
  setBudget() {
    ...
    if (!form) {
      this.showToast("Form anggaran tidak ditemukan", "error");
      return;
    }
    ...
    if (!amountInput || !monthInput) {
      this.showToast("Field anggaran tidak lengkap", "error");
      return;
    }

    // Get raw value or clean the formatted value
    let raw = amountInput.dataset.rawValue || amountInput.value || "";
    const cleaned = raw.replace(/[^0-9]/g, "");
    const amount = cleaned ? parseFloat(cleaned) : 0;
    const selectedMonth = monthInput.value;

    if (!selectedMonth) {
      this.showToast("Silakan pilih bulan terlebih dahulu", "error");
      return;
    }

    if (amount <= 0) {
      this.showToast(
        "Masukkan jumlah anggaran yang valid (lebih dari 0)",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      this.budget[selectedMonth] = amount;

      // If setting budget for current month, also update user settings
      ...
      this.saveUserData();
      this.showToast(
        `Anggaran untuk ${selectedMonth} berhasil diatur: ${Utils.formatCurrency(
          amount
        )}`,
        "success"
      );
      ...
      // Force comprehensive budget update
      this.refreshBudgetDisplay();

      console.log("Budget updated - New stats:", {
        monthlyBudget: this.stats.monthlyBudget,
        budgetRemaining: this.stats.budgetRemaining,
        budgetPercentage: this.stats.budgetPercentage,
      });
    } catch (error) {
      console.error("Error setting budget:", error);
      this.showToast("Terjadi kesalahan saat menyimpan anggaran", "error");
    }
  },
  ...
  updateHistory() {
    console.log(
      "updateHistory called, transactions count:",
    ...
    if (filteredTransactions.length === 0) {
      console.log(
        "No filtered transactions found. Total transactions:",
        this.transactions.length
      );
      transactionList.innerHTML = `
        <div class="empty-state-modern">
          ...
          <button class="btn-primary-modern" onclick="window.location.href='add-transaction.html'">
    ...
    transactionList.innerHTML = filteredTransactions
      .map(
        (t) => `
      <div class="transaction-card-modern ${t.type}" data-id="${t.id}">
        <div class="transaction-card-content">
          <div class="transaction-icon-modern ${t.type}">
          ...
          <div class="transaction-details">
            <div class="transaction-header">
              <h4 class="transaction-title-modern">${
                this.categories[t.type]?.[t.category]?.name || "Lainnya"
              }</h4>
              <span class="transaction-amount-modern ${t.type}">
                ${t.type === "expense" ? "-" : "+"}${Utils.formatCurrency(
          Math.abs(t.amount)
        )}
              </span>
            </div>
            <div class="transaction-meta">
              <span class="transaction-date-modern">
              ...
              <span class="transaction-time-modern">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            ${
              t.description
                ? `<div class="transaction-description-modern">${t.description}</div>`
                : ""
            }
          </div>
        </div>
        <div class="transaction-actions-modern">
          <button class="action-btn-modern edit-btn" onclick="window.app.editTransaction('${
            t.id
          ...
          <button class="action-btn-modern delete-btn" onclick="window.app.deleteTransaction('${
            t.id
  ...
  updateDashboard() {
    console.log("updateDashboard: Refreshing dashboard view.");
  ...
  updateDashboardStats(maxRetries = 3, retryDelay = 100) {
  ...
  updateBudget() {
    ...
    if (!budgetSection) {
      console.log("Budget section not found in DOM");
      return;
    }

    // Make sure we have fresh stats
    this.updateGlobalStats();

    // Gunakan data dari stats global
    const stats = this.stats;

    console.log("Updating budget display with stats:", {
  ...
  refreshBudgetDisplay() {
    ...
    setTimeout(() => {
      ...
      // Force update dashboard stats elements
      this.updateDashboardStats();
    }, 50);
  },
