...
const ExpenseTracker = {
  ...
  saveUserData() {
    ...
    // Ensure current month's budget is synchronized with user settings
    const currentMonth = new Date().toISOString().substring(0, 7);
    if (this.budget[currentMonth] !== undefined) {
      if (!this.currentUser.settings) {
        this.currentUser.settings = {};
      }
      this.currentUser.settings.monthlyBudget = this.budget[currentMonth];
      localStorage.setItem("currentUser", JSON.stringify(this.currentUser));
    }
  },
  ...
  updateAllCategoryDropdowns() {
    ...
    // Category filter is handled separately
    if (typeof this.populateCategoryFilter === 'function') {
  ...
  setBudget() {
    ...
    if (amount >= 0) {
      // Update budget in both places to ensure synchronization
      this.budget[currentMonth] = amount;

      // Also update in user settings for the current month
      if (!this.currentUser.settings) {
        this.currentUser.settings = {};
      }
