...
const ExpenseTracker = {
  ...
  updateRecentTransactions() {
    ...
    container.innerHTML = recent.length ? recent.map(t => `
      <div class="transaction-item ${t.type}" data-id="${t.id}">
        <div class="transaction-icon">${this.categories[t.type]?.[t.category]?.icon || '📝'}</div>
        <div class="transaction-info">
          <div class="transaction-title">${this.categories[t.type]?.[t.category]?.name || 'Lainnya'}</div>
          <div class="transaction-date">${Utils.formatDate(t.date)} ${t.time}</div>
          ${t.description ? `<div class="transaction-desc">${t.description}</div>` : ''}
        </div>
        <div class="transaction-amount ${t.type}">${Utils.formatCurrency(t.amount)}</div>
      </div>
  ...
  updateDashboardStats() {
    ...
    const monthTransactions = this.transactions.filter(t => t.date.startsWith(thisMonth));
    ...
    if (statsContainer) {
      statsContainer.innerHTML = `
        ...
        <div class="stat-card balance">
          ...
          <div class="stat-value ${balance >= 0 ? 'positive' : 'negative'}">${Utils.formatCurrency(balance)}</div>
        </div>
      `;
    }
  },
  ...
  updateSpendingChart() {
    ...
    const data = {
      labels: Object.keys(expenseByCategory).map(cat => this.categories.expense?.[cat]?.name || cat),
      datasets: [{
        data: Object.values(expenseByCategory),
        backgroundColor: Object.keys(expenseByCategory).map(cat => this.categories.expense?.[cat]?.color || '#666'),
      }]
    };
  ...
  updateUserInfo() {
    ...
    if (this.currentUser) {
      ...
      if (guestBadgeEl) {
        guestBadgeEl.style.display = this.currentUser.isGuest
          ? "inline"
          : "none";
      }
      ...
      if (statsEl) {
        statsEl.innerHTML = `
          ...
          <div class="stat-item">
            ...
            <span class="stat-value income">${Utils.formatCurrency(
              stats.totalIncome
            )}</span>
          </div>
          <div class="stat-item">
            <span class="stat-label">Total Pengeluaran</span>
            <span class="stat-value expense">${Utils.formatCurrency(
              stats.totalExpense
            )}</span>
          </div>
          <div class="stat-item">
            ...
            }">${Utils.formatCurrency(stats.balance)}</span>
          </div>
        `;
      }
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.NotificationSystem = window.NotificationSystem || {
    addNotification: function(type, message, amount) {
      ExpenseTracker.showToast(message, type === 'success' ? 'success' : 'info');
    }
  };
