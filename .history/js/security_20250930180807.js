...
const Security = {
  ...
  storeKey: async function(key) {
    ...
    sessionStorage.setItem(`key_${keyId}`, Array.from(new Uint8Array(exportedKey)).map(b => b.toString(16).padStart(2, '0')).join(''));
  ...
  checkLoginAttempts: function(email) {
    ...
    if (attempts.count >= this.config.maxLoginAttempts) {
      ...
      if (remainingTime > 0) {
        throw new Error(`Terlalu banyak percobaan login. Coba lagi dalam ${Math.ceil(remainingTime / 60000)} menit.`);
      }
    }
