...
window.ExpenseTracker = {
  ...
  populateCategoryFilter() {
    ...
    if (currentValue && currentValue !== "") {
      ...
      const optionExists = [...filterSelect.options].some(option => option.value === currentValue);
      if (optionExists) {
  ...
  addTransaction() {
    ...
    setTimeout(() => {
      this.updateDashboard();
      this.updateBudget();
      this.updateBudgetStatus(); // Force update elemen-elemen penting dengan selector yang benar
      const headerBalance = document.getElementById("headerBalance");
      const dashboardBalance = document.getElementById("dashboardBalance");

      if (headerBalance) {
        headerBalance.textContent =
          this.stats.monthlyBalance.toLocaleString("id-ID");
      }
      ...
      if (monthIncomeElement) {
        monthIncomeElement.textContent = Utils.formatCurrency(
          this.stats.monthlyIncome
        );
      }

      if (monthExpenseElement) {
        monthExpenseElement.textContent = Utils.formatCurrency(
          this.stats.monthlyExpense
        );
      }
  ...
  updateHistory() {
    ...
    const needCategoryPopulate = !categoryFilter || categoryFilter.children.length <= 1;

    if (needMonthPopulate && this.transactions.length > 0) {
      this.populateMonthFilter();
    }

    if (needCategoryPopulate && (this.categories.expense || this.categories.income)) {
      this.populateCategoryFilter();
    }
  ...
  updateBalance() {
    // Update both header balance and main dashboard balance
    const headerBalance = document.getElementById("headerBalance");
    const dashboardBalance = document.getElementById("dashboardBalance");
    ...
    if (headerBalance) {
      headerBalance.textContent =
        this.stats.monthlyBalance.toLocaleString("id-ID");
      console.log("Header balance updated:", this.stats.monthlyBalance);
    }
    ...
    if (!headerBalance && !dashboardBalance) {
