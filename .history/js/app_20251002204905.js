    ...
    const attemptUpdate = (attempt = 0) => {
      try {
        ...
        if (missingElements.length > 0) {
          console.log(`updateDashboardStats: Missing elements: ${missingElements.join(", ")}`);
          if (attempt < maxRetries - 1) {
            setTimeout(() => attemptUpdate(attempt + 1), retryDelay);
            return;
          }
          console.error("updateDashboardStats: Failed to find all required elements after all attempts");
          return;
        }
