...
window.Utils = {
  formatCurrency: (amount, options = {}) => {
    ...
    if (window.innerWidth <= 480 && absAmount >= 1000000) {
      if (absAmount >= 1000000000) {
        ...
        const shortValue = currentLang === "id" ? 
  ...
  formatRelativeTime: (date) => {
    ...
    if (diffMinutes < 1) {
      ...
      return currentLang === "id" ? `${diffMinutes} menit lalu` : `${diffMinutes} minutes ago`;
    } else if (diffHours < 24) {
      return currentLang === "id" ? `${diffHours} jam lalu` : `${diffHours} hours ago`;
    } else if (diffDays === 1) {
      return currentLang === "id" ? "Kemarin" : "Yesterday";
    } else if (diffDays < 7) {
      return currentLang === "id" ? `${diffDays} hari lalu` : `${diffDays} days ago`;
    } else {
...
window.ExpenseTracker = {
  ...
  addTransaction() {
    ...
    if (!amount || amount <= 0) {
      this.showToast("Mohon masukkan jumlah yang valid", "error", "pleaseEnterValidAmount");
      return;
    }
    if (!category) {
  ...
  deleteTransaction(id) {
    if (confirm("Apakah Anda yakin ingin menghapus transaksi ini?")) {
      ...
      this.saveUserData();
      this.updateUI();
      this.showToast("Transaksi berhasil dihapus!", "success", "transactionDeletedSuccessfully");
    }
  },
  ...
  setBudget() {
    ...
    if (amount >= 0) {
      ...
      this.saveUserData();
      this.showToast("Anggaran berhasil diatur!", "success", "budgetSetSuccessfully");
  ...
  logout() {
    const confirmMessage = window.i18n ? window.i18n.t("confirmLogout") : "Apakah Anda yakin ingin keluar?";
