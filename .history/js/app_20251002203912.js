...
window.ExpenseTracker = {
    ...
    const attemptUpdate = (attempt) => {
      try {
        ...
        if (!dashboardSection || !dashboardSection.classList.contains('active')) {
          console.log(`Attempt ${attempt + 1}: Dashboard not active, skipping balance update`);
          return false;
        }
        ...
        if (dashboardBalance) {
          ...
          console.log(
            `Dashboard balance updated successfully on attempt ${attempt + 1}: ${formattedBalance}`
          );
          return true;
        } else {
          console.warn(
  ...
  addTransaction() {
    ...
    // Important: Calculate stats BEFORE switching to dashboard
    this.updateGlobalStats();

    // Use a sequence of timeouts to ensure proper transitions
    // This prevents the blank page issue by giving the browser time to process each step
    setTimeout(() => {
      console.log("Transaction added, preparing to switch to dashboard");

      // First prepare the stats and dashboard data
      this.updateDashboardStats();

      // Then switch to dashboard with another small delay
      setTimeout(() => {
  ...
  updateDashboardStats() {
    try {
      console.log('updateDashboardStats: Starting update process');
      ...
      const dashboardSection = document.getElementById('dashboard');
      if (!dashboardSection || !dashboardSection.classList.contains('active')) {
        console.log('updateDashboardStats: Dashboard section not active, skipping update');
        return;
      }

      // Update all dashboard elements in a consistent order
      const dashboardElements = {
        dashboardBalance: { value: stats.monthlyBalance },
        monthIncome: { value: stats.monthlyIncome },
        monthExpense: { value: stats.monthlyExpense },
      };

      // Update each element if it exists
      for (const [elementId, data] of Object.entries(dashboardElements)) {
        const element = document.getElementById(elementId);
        if (element) {
          const formattedValue = Utils.formatCurrency(data.value);
          element.textContent = formattedValue;
          console.log(`updateDashboardStats: Updated ${elementId} to ${formattedValue}`);
        } else {
          console.warn(`updateDashboardStats: Element ${elementId} not found in DOM`);
        }
      }

      // Also update the main stats display if it exists
      const mainStatsDisplay = document.getElementById("dashboardStats");
      if (mainStatsDisplay) {
        mainStatsDisplay.innerHTML = `
          ...
          <div class="stat-card balance">
            ...
            <div class="stat-value ${
              ...
              ${Utils.formatCurrency(stats.monthlyBalance)}
            </div>
          </div>
        `;
      }
