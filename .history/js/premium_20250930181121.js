...
const PremiumFeatures = {
  ...
  getCurrentCustomCategoriesCount: function() {
    ...
    const customCategories = Object.keys(user.settings.categories.expense || {})
  ...
  isDefaultCategory: function(categoryKey) {
    const defaultCategories = ['makanan', 'transportasi', 'belanja', 'hiburan', 'kesehatan', 'pendidikan', 'tagihan', 'lainnya'];
  ...
  getTransactionHistoryDays: function() {
    ...
    const daysDiff = Math.ceil((new Date() - new Date(oldestTransaction.date)) / (1000 * 60 * 60 * 24));
    return daysDiff;
  },
  ...
  getFeatureDescription: function(featureName) {
    const descriptions = {
      maxCustomCategories: 'Buat kategori kustom tanpa batas untuk pencatatan yang lebih detail.',
      maxTransactionHistory: 'Akses riwayat transaksi lengkap tanpa batasan waktu.',
      exportFormats: 'Ekspor data ke berbagai format (CSV, PDF, Excel) untuk analisis lebih lanjut.',
  ...
  simulateSuccessfulPayment: function(planType) {
    ...
    this.closePremiumPrompt();
    showToast('🎉 Selamat! Akun Anda telah diupgrade ke Premium!', 'success', 5000);
  ...
  enforceTransactionHistoryLimit: function(transactions) {
    ...
    return transactions.filter(transaction => 
      new Date(transaction.date) >= limitDate
    );
  },
