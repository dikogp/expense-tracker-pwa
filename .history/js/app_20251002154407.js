...
window.ExpenseTracker = {
  ...
  setBudget() {
    ...
    if (amount >= 0) {
      // The budget for the selected month is updated.
      this.budget[currentMonth] = amount;

      // If the budget being set is for the CURRENT month, also update the
      // master monthlyBudget in user settings. This is the single source of truth.
      ...
      // Save both the budget object (for all months) and the currentUser object.
      this.saveUserData();

      this.showToast("Anggaran berhasil diatur!", "success");
      window.closeModal("budgetModal");
      this.updateUI(); // This will re-run all calculations and update the display.
    } else {
  ...
  updateUI() {
    // Sync user settings with budget data for consistency
    if (this.currentUser && this.currentUser.settings) {
      const thisMonth = new Date().toISOString().substring(0, 7);

      // Initialize budget object if needed
      if (!this.budget) {
        this.budget = {};
      }

      // Two-way synchronization between budget object and user settings
      if (
        this.currentUser.settings.monthlyBudget !== undefined &&
        (this.budget[thisMonth] === undefined ||
          this.budget[thisMonth] !== this.currentUser.settings.monthlyBudget)
      ) {
        // User settings has priority if budget object is unset
        this.budget[thisMonth] = this.currentUser.settings.monthlyBudget;
      } else if (
        this.budget[thisMonth] !== undefined &&
        this.currentUser.settings.monthlyBudget !== this.budget[thisMonth]
      ) {
        // Budget object has priority if both exist but differ
        this.currentUser.settings.monthlyBudget = this.budget[thisMonth];
        localStorage.setItem("currentUser", JSON.stringify(this.currentUser));
      }
    }
  ...
  updateSpendingChart() {
    ...
    monthExpenses.forEach((t) => {
      expenseByCategory(expenseByCategory[t.category] || 0) + t.amount;
    });
